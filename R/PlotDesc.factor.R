...
function (x, main = deparse(substitute(x)), 
                ...
                type=c("bar","dot"), col=hblue, ..., wrd=NULL)  {
  ...
  if (nlevels(factor(x)) <= 2) {
    ...
    par(mai=c(1, max(strwidth(rev(rownames(tab)), "inch"))+.5, 0.2, .3)+.02) 
    ...
    switch(match.arg(arg = type, choices = c("bar", "dot")), 
           dot = {
             ...
             points( x=rev(tab), y=b, yaxt = "n", col="black", pch=21, bg=col, cex=1.3)
             box()  
             
             par(mai=c(1, 0.1, 0.2, .3)+.02) 
             ...
             abline(h=b, v=0, col="grey", lty="dotted")
             points( x=rev(ptab), y=b, col="black", pch=21, bg=col, cex=1.3)
             box()
             
           }, bar = { # type = "bar" 
             barplot( rev(tab), horiz=TRUE, border=NA, col="grey80", las=1, xlim=range(pretty(tab)), xpd=FALSE, xlab="frequency", cex.names=cex, cex.axis=cex, cex.lab=cex, tck=-0.04)
             grid(ny=NA)
             
             par(mai=c(1, 0.1, 0.2, .3)+.02) 
             barplot( rev(ptab), horiz=TRUE, border=NA, col="grey80", las=1, names="", xlim=c(0,1), xlab="percent", cex.names=cex, cex.axis=cex, cex.lab=cex, tck=-0.04)
             grid(ny=NA)
    ...
    if(!is.null(wrd)) WrdPlot(width=8, height=5, dfact=2.5,  wrd=wrd, append.cr=FALSE)
