...
function(tab, acol = rainbow(sum(dim(tab))), aborder = "darkgrey",
  ...
  ribbon <- function( angle1.beg, angle1.end, angle2.beg, angle2.end, 
    ...
    bez1 <- DrawArc(radius.x = radius2, angle.beg = CartToPol(xy2$x, xy2$y)$theta, angle.end = CartToPol(xy4$x, xy4$y)$theta, plot=FALSE)[[1]]
    bez2 <- DrawBezier( x = c(xy4$x, 0, xy3$x), y = c(xy4$y, 0, xy3$y), plot=FALSE )
    bez3 <- DrawArc(radius.x = radius1, angle.beg=CartToPol(xy3$x, xy3$y)$theta, angle.end=CartToPol(xy1$x, xy1$y)$theta, plot=FALSE )[[1]]
    bez4 <- DrawBezier( x = c(xy1$x, 0, xy2$x), y = c(xy1$y, 0, xy2$y), plot=FALSE )
  ...
  Canvas(10, main=main, xpd=TRUE)
  DrawAnnulusSector(x=0, y=0, radius.in=9.5, radius.out=10, 
